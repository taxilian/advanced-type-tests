...
export type TypeEquals<X, Y> =
    Or<BothBool<X, Y>, BothAny<X, Y>> extends Yes ? Yes :
...
// Note that in order to compare this exactly you have to wrap it as a tuple
type t_TypeEqualsSupportsStringLitUnion = TypeEquals<['true' | 'false'], ['true' | 'false']>;
...
type TESTAGG_TypeEqualsTests = Yes
    ...
    & t_TypeEqualsSupportsStringLitUnion;
...
export type FieldsCheckEqual<X, Y, Keys extends keyof X | keyof Y = keyof X | keyof Y> = {
    [Key in Keys]: [Key] extends [FilterKeys<X>] ? [Key] extends [FilterKeys<Y>] ? (
        [X[Key]] extends [any][] ? 
...
export type FieldsCheckEqual_Test<X, Y, Keys extends keyof X | keyof Y = keyof X | keyof X> = {
    [Key in Keys]: [[Key] extends [FilterKeys<X>] ? [Key] extends [FilterKeys<Y>] ? (
        [X[Key]] extends [any[]] ?
        ArrayTypeEquals<[X[Key]], [Y[Key]]> : TypeEquals<[X[Key]], [Y[Key]]>
        ) : No : No, Key extends keyof X ? X[Key] : false, Key extends keyof Y ? Y[Key] : false];
};
...
interface testFieldCheckExtraAny extends testFieldCheck {
    meh: any;
}
...
type TestCompareBad1 = FieldsCheckEqual<testFieldCheckExtra, testFieldCheck>;
type TestCompareBad2 = FieldsCheckEqual<testFieldCheck, testFieldCheckExtraAny>;
type TestCompareBad3 = FieldsCheckEqual<testFieldCheckExtra, testFieldCheck>;
type TestCompareBad4 = FieldsCheckEqual<testFieldCheckWithNever, testFieldCheckWithNever>;
...
type t_TestFieldCompareBad4 = Invert<FieldsConfirmCheck<TestCompareBad4>>;
